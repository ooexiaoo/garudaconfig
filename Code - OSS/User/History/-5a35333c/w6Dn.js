...
document.addEventListener('keydown', function(event) {
    // Check if the pressed key is a number from 1 to 7
    if (event.key >= '1' && event.key <= '7') {
        // Get the corresponding section based on the pressed key
        const sectionId = document.querySelector(`#menu-content p[id="number"][id="number"]:contains(${event.key})`).closest('.inner-section').getAttribute('data-section');
        
        // Toggle the section corresponding to the pressed key
