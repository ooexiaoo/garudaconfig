async function fetchPrices() {
    const response = await fetch('https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=1000&page=1&sparkline=true');
...
function displayPrices(prices) {
    ...
    prices.forEach(crypto => {
        ...
        priceItem.addEventListener('click', () => {
            window.location.href = `detail.html?id=${crypto.id}`;
        });
