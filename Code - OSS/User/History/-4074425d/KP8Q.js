themeButtons.forEach((color, index) => {
  color.addEventListener("click", () => {
    ...
    // Check if the clicked button is the color picker button (case 20)
    if (index === 20) {
      // Retrieve the selected color from the color picker
      let selectedColor = colorPicker.color.hexString;
      // Check if local storage background image exists
      let localStorageBg = localStorage.getItem("background_image");
      // If local storage background image exists, use it along with the selected color
      if (localStorageBg) {
        dataColor = localStorageBg + ", " + selectedColor;
      } else {
        // Otherwise, only use the selected color
        dataColor = selectedColor;
      }
    } else {
      // For other cases, retrieve the data-color attribute of the clicked button
      dataColor = color.getAttribute("data-color");
    }

    // Apply the dataColor to --pomodoro
