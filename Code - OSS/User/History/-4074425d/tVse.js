themeButtons.forEach((color, index) => {
  color.addEventListener("click", () => {
    // Retrieve the data-color attribute from localStorage or set it to black if not found
    let dataColor = localStorage.getItem(`backgroundImage${index}`) || "#000000";

    // Apply the dataColor to --pomodoro
    document.documentElement.style.setProperty(
      "--pomodoro",
      dataColor ? `url(${dataColor})` : "black"
    );
