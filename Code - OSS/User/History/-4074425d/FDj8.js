themeButtons.forEach((color, index) => {
  color.addEventListener("click", () => {
    ...
    // Check if the clicked button is the color picker button
    if (color.classList.contains("color-picker-button")) {
      // If case 20 is clicked, retrieve the local storage background image
      if (index === 20) {
        dataColor = localStorage.getItem("backgroundImage");
      } else {
        // Otherwise, set dataColor to the value of data-color attribute
        dataColor = color.getAttribute("data-color");
      }
    } else {
      // If it's not the color picker button, set dataColor to the value of data-color attribute
      dataColor = color.getAttribute("data-color");
    }
