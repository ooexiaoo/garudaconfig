...
const timer = {
  ...
  themeButtons.forEach(color => {
    color.addEventListener('click', () => {
      ...
      // Check if the clicked button is the one triggering case 20 (color picker button)
      if (color.classList.contains('color-picker-button')) {
        document.querySelector(':root').style.setProperty('--Red', none);
        return;
      }
  
      // Set --pomodoro based on data-color attribute
