...
colorPicker.on('color:change', function(color) {
  try {
    ...
    // Check if customBackgroundUrl is not null and not 'none'
    if (customBackgroundUrl && customBackgroundUrl !== 'none') {
      ...
      document.querySelector(':root').style.setProperty('--pomodoro', color.hexString);
      console.log('Selected color applied:', color.hexString);
    }
  } catch (error) {
    // Handle error while accessing localStorage data
