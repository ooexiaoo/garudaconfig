...
colorPicker.on('color:change', function(color) {
  ...
  if (color.hexString === 'none') {
    ...
    document.querySelector(':root').style.setProperty('--Red', color.hexString);
    
    try {
      // Access the localStorage data
      const timerData = JSON.parse(localStorage.getItem('timerData'));
      console.log('found data', timerData);
      
      // Check if localStorage data is not null and contains a customBackgroundUrl property
      if (timerData && timerData.customBackgroundUrl) {
        const customBackgroundUrl = timerData.customBackgroundUrl;
        document.querySelector(':root').style.setProperty('--pomodoro', `url(${customBackground})`);
        console.log('customBackgroundUrl', customBackground);
      }
    } catch (error) {
      // Handle error while accessing localStorage data
